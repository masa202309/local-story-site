    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {stories.length === 0 ? (
          ...
          <>
            {/* フィーチャーストーリー */}
            <Link href={`/story/${stories[0].id}`} className="block mb-6">
              <article className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition md:flex">
                <div className="md:w-2/5">
                  <img
                    src={stories[0].image_url || "https://images.unsplash.com/photo-1559925393-8be0ec4767c8?w=800"}
                    alt={stories[0].shops?.name || ""}
                ...
                <div className="p-4 md:w-3/5 md:p-6">
                  <div className="flex items-center gap-2 text-xs text-amber-600 mb-2">
                    <span className="bg-amber-100 px-2 py-0.5 rounded">
                      {stories[0].shops?.area}
                    </span>
                    <span>{stories[0].shops?.genre}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      ...
                      <span>{stories[0].shops?.name}</span>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1">
                        ❤️ {stories[0].reactions_warm}
                      </span>
                    </div>
                  </div>
                </div>
